// import { useTonalPalette } from '../src/theme/content/hooks/useTonalPalette';
// import { generateDesignTokens } from '../src/theme/content/utils/generateTokens';

import { argbFromHex, hexFromArgb, themeFromSourceColor } from '@material/material-color-utilities';
// import { console } from 'inspector';

function getColorToken(themeMode, palette, lightLevel, darkLevel) {
  return palette[themeMode == 'light' ? lightLevel : darkLevel];
}

const main = async () => {
  const LEVELS = [0, 4, 6, 10, 12, 17, 20, 22, 24, 30, 40, 50, 60, 70, 80, 87, 90, 92, 94, 95, 96, 98, 99, 100];
  //   const TONAL_PALETTE_KEY = 'tonalPalette';
  console.time('Seeding complete 🌱');
  // const [tonalPalette] = useTonalPalette();
  // const lightTokens = generateDesignTokens('light', tonalPalette);
  // const darkTokens = generateDesignTokens('dark', tonalPalette);
  // console.log("lightTokens", lightTokens)
...
const themeMode = 'dark';
  ...
  console.log('Tonal Palette generated successfully');
//   console.log('Tonal Palette', JSON.stringify(tones));

  //   const getColorToken = (themeMode, palette, lightLevel, darkLevel) =>
  //     palette[themeMode == 'light' ? lightLevel : darkLevel];

  const tonalPalette = tones;
...
// Run the script with the following command: npx ts-node scripts\generatePalette.mjs