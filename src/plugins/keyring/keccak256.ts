...
export default {
  ...
  publicToID(publicKey: string): string {
    if (publicKey.startsWith('04')) {
      ...
      const keyDigest = CryptoJS.enc.Hex.parse(sha3.keccak256(convert.toArrayBuffer(publicKey.slice(2)) as any));
      ...
      const keyId = crcLong.sub(keyring.remainder(crc, 10)).add(addrChecksum).toString();
      return keyId;
    }
