...
export const getMUIPalette = (themeMode: ThemeMode, themescheme: ThemeScheme): ThemeOptions => {
  ...
  return {
    palette: {
      ...
      // Custom Colors
      container: {
        main: themeMode === 'light' ? '#fbf8fd' : '#111827',
        contrastText: themeMode === 'light' ? '#6b7280' : '#9ca3af'
      },
      onBody: {
        main: themeMode === 'light' ? '#f3f4f6' : '#1f2937'
      },
      onTitle: {
        main: themeMode === 'light' ? '#1f2937' : '#9ca3af'
      },
      onBorder: {
        main: themeMode === 'light' ? '#e5e7eb' : '#374151'
      },
      onPrimary: {
